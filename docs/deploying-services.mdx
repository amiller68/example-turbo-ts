...
2. Create a hosted zone for your domain (e.g., `krondor.org`)
3. Create a subdomain hosted zone for AWS services:
   ```bash
   aws route53 create-hosted-zone \
     --name aws.krondor.org \
     --caller-reference $(date +%s)
   ```
4. Note the NS records from the subdomain zone
5. In the parent zone (`krondor.org`), create NS records for the subdomain:
   - Name: `aws`
   - Type: `NS`
   - Values: (the four NS values from step 4)

The infrastructure expects this hosted zone to exist at `aws.krondor.org` as referenced in `iac/modules/aws/main.tf`.
