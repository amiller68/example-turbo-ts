...
# Push using Infisical for credentials
./bin/docker.sh push your-service-name -I

# (Most likely) push to staging, with `staging-latest`, no branch protection,
#  and allowing pushes against dirty branches
./bin/docker.sh push your-service-name -I -D -L -S
```
...
For pushing images, you'll need AWS credentials. These can be provided:

- As environment variables (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_REGION`)
- Via Infisical with the `-I` flag (recommended for development)
